...
# 初始化状态文件（如果不存在）
STATE_FILE="$OPENCLAW_HOME/insight-state.json"
...
configure_cron() {
    ...
    if ! command -v crontab &> /dev/null; then
        echo -e "${YELLOW}⚠ crontab 不可用，跳过定时任务配置${NC}"
        return 0
    fi
    ...
    # 添加定时任务
    echo "# Insight System - AI 自我驱动" >> "$CRON_TEMP"
    echo "0 */3 * * * cd $INSIGHT_DIR && python3 core/auto_driver.py >> $OPENCLAW_HOME/logs/auto_drive.log 2>&1" >> "$CRON_TEMP"

    # 安装 crontab
    crontab "$CRON_TEMP"
    rm "$CRON_TEMP"

    echo -e "${GREEN}✓ crontab 任务已配置：每3小时运行自我驱动${NC}"
}

# 询问用户是否配置 crontab
read -p "是否配置定时自动驱动任务? (y/n): " CONFIGURE_CRON
...
echo ""
echo "4. 手动运行自我驱动:"
echo "   python core/auto_driver.py"
echo ""
echo "5. 查看定时任务:"
echo "   crontab -l"
echo ""
